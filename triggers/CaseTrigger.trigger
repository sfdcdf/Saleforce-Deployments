 ...
 * Lines #387
 * Introduced the Check recursive to avoid recursive loops
 */
...
trigger CaseTrigger on Case (before insert, before update, after insert, after update) {
    ...
    if(trigger.isBefore){
        ...
        if(trigger.isInsert){
            ...
            if(trigger.isUpdate){
                if(CheckRecursive.runAfterUpdate()){
            ...
            list<TP_Section_Auto_Evaluate_Criteria__c> List_TP =[select Id, Touchplan_Section__r.Touchplan__c, Touchplan_Section__r.Section_Order__c, 
                                                                 Touchplan_Section__r.Case__c, Touchplan_Section__r.Case__r.Status, 
                                                                 Touchplan_Section__r.Set_Next_Section_Evaluation_Date__c from TP_Section_Auto_Evaluate_Criteria__c 
                                                                 where Touchplan_Section__r.Case__c in : CaseIdset];
            for(TP_Section_Auto_Evaluate_Criteria__c ypCriteria : List_TP){
                Case currentCase = trigger.newMap.get(ypCriteria.Touchplan_Section__r.Case__c);
                Case oldCase = trigger.oldMap.get(ypCriteria.Touchplan_Section__r.Case__c);
